package com.huberlin.javacep.util.testing;
...
public class LoopbackTCPConnection {
  ...
  public LoopbackTCPConnection() throws IOException, InterruptedException {
    ...
    try {
      accepting_thread.join(
          1000); // FIXME: Reliably wait for ssc to be ready to accept connections (this is still a
                 // race condition! it can take more than 1s for the other thread to get scheduled)
   ...
   * @throws IOException
   */
  public Thread transmit_data(
      byte[] data_to_transmit, long start_after_ms, boolean close_afterwards) throws IOException {
    Thread tx_thread =
        new Thread(
            () -> {
              try {
                ...
                if (close_afterwards) tx_socket.close();
